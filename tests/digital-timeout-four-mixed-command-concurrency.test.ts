...
test('four Digital clients racing different commands on one revision accept exactly one and stay synchronized', async () => {
  ...
  try {
    ...
    const clientIndexForSeat = (seat: number) => {
      const playerId = timeoutCanonical.players[seat].id,
        clientIndex = users.findIndex((user) => user.profile.id === playerId);
      assert.ok(clientIndex >= 0);
      return clientIndex;
    };
    const activeClientIndex = clientIndexForSeat(1);

    // Seed a draw offer from the actual active seat. Private-room seats are shuffled.
    const seedCommandId = randomUUID(),
      seedWaiters = clients.map((client) =>
        client.next(
          (message) =>
            message.type === 'match' &&
            message.match.id === matchId &&
    ...
    clients[activeClientIndex].send({
      type: 'draw-offer',
      matchId,
    ...
    const answerClients = [0, 1, 2, 3].filter((index) => index !== activeClientIndex);
    assert.equal(answerClients.length, 3);

    // Four distinct command objects race the same expectedRevision=2.
    // Each is independently valid and non-terminal at revision 2:
    // one active-seat move, one draw acceptance, one draw rejection, and another acceptance
    // from a different player/command identity.
    const actions = [
      {
        clientIndex: activeClientIndex,
    ...
    assert.equal(new Set(actions.map((action) => action.commandId)).size, 4);
    assert.deepEqual(new Set(actions.map((action) => action.clientIndex)), new Set([0, 1, 2, 3]));
      ...
      actions.map(
        (action) =>
          new Promise<void>((resolve) =>
            setImmediate(() => {
              clients[action.clientIndex].send({ ...action.message, commandId: action.commandId });
              resolve();
            }),
          ),
      ),
    ...
    if (acceptedAction!.message.type === 'move') {
      ...
      assert.notEqual(acceptedSeat, 1);
      ...
      assert.deepEqual(canonical.drawAccepts, [1, acceptedSeat]);
    } else {
      assert.notEqual(acceptedSeat, 1);
      assert.equal(canonicalState.turn, 1);
      assert.equal(canonicalState.ply, 1);
