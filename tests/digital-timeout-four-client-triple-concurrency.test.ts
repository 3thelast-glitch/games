...
test('three of four Digital clients racing the same revision accept exactly one action after timeout', async () => {
  ...
  try {
    const users = await Promise.all([
        guest('TripleRaceAlice4'),
        guest('TripleRaceBob4'),
        guest('TripleRaceCarol4'),
        guest('TripleRaceDave4'),
      ]),
    ...
    for (const match of afterTimeout) {
      const projected = match.state as DigitalGameState;
      assert.deepEqual(sharedSnapshot(match), timeoutBaseline);
    ...
    // Three different clients submit the same revision-guarded action concurrently.
    // Exactly one can advance revision 1 -> 2; the two remaining commands must be stale.
    const contenderIndexes = [1, 2, 3] as const,
    ...
    assert.equal(contenderErrors.length, 2);
